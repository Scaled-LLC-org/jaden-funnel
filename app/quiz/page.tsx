...
const SCREENS: Screen[] = [
  ...
  },
  {
    ...
    stats: [{ n: "100%", l: "Natural" }, { n: "300+", l: "Workouts" }, { n: "1:1", l: "With Jaden" }],
  },
  {
    type: "single",
    key: "situation",
...
const COUNTRIES = [
  ...
  { c: "AU", d: "+61", f: "🇦🇺" }, { c: "IE", d: "+353", f: "🇮🇪" }, { c: "NZ", d: "+64", f: "🇳🇿" },
  { c: "DE", d: "+49", f: "🇩🇪" }, { c: "FR", d: "+33", f: "🇫🇷" }, { c: "ES", d: "+34", f: "🇪🇸" },
  { c: "IT", d: "+39", f: "🇮🇹" }, { c: "NL", d: "+31", f: "🇳🇱" }, { c: "SE", d: "+46", f: "🇸🇪" },
  { c: "NO", d: "+47", f: "🇳🇴" }, { c: "DK", d: "+45", f: "🇩🇰" }, { c: "CH", d: "+41", f: "🇨🇭" },
  { c: "AE", d: "+971", f: "🇦🇪" }, { c: "SG", d: "+65", f: "🇸🇬" }, { c: "ZA", d: "+27", f: "🇿🇦" },
  { c: "MX", d: "+52", f: "🇲🇽" }, { c: "BR", d: "+55", f: "🇧🇷" }, { c: "IN", d: "+91", f: "🇮🇳" },
];
...
function QuizInner() {
  ...
  const back = () => { setMulti([]); setI((n) => Math.max(n - 1, 0)); };
  ...
  const toggleMulti = (value: string) =>
    setMulti((m) => (m.includes(value) ? m.filter((v) => v !== value) : [...m, value]));
  ...
  const finish = () => {
    const params = new URLSearchParams({ ...answers, name, email, phone: `${dialCode} ${phone}` }).toString();
  ...
  return (
    <main style={{ minHeight: "100vh", background: "var(--bg)", display: "flex", flexDirection: "column" }}>
      ...
      <section style={{ flex: 1, display: "flex", alignItems: screen.type === "analyzing" ? "center" : "flex-start", justifyContent: "center", padding: "40px 0 64px" }}>
        <div className="am-wrap am-fade" key={i} style={{ maxWidth: 600, width: "100%" }}>

          {/* INTRO */}
          {screen.type === "intro" && (
            <div style={{ textAlign: "center" }}>
              <p className="am-eyebrow" style={{ marginBottom: 14 }}>Get your personalized</p>
              ...
              <p className="am-serif" style={{ fontSize: 20, fontWeight: 500, color: "var(--ink-deep)", marginBottom: 18 }}>{screen.q}</p>
              <div style={{ display: "flex", flexDirection: "column", gap: 12, textAlign: "left", maxWidth: 460, margin: "0 auto" }}>
                {screen.opts.map((o) => <OptCard key={o.value} o={o} onClick={() => pick(undefined, o.value)} />)}
              </div>
          ...
          {screen.type === "single" && (
            <div>
              <h2 className="am-h2" style={{ fontSize: 30, margin: "0 0 10px" }}>{screen.q}</h2>
          ...
          {screen.type === "proof" && (
            <div style={{ textAlign: "center" }}>
              ...
              <h2 className="am-h2" style={{ fontSize: 28, margin: "0 0 14px" }}>{screen.headline}</h2>
              ...
              <div style={{ display: "flex", justifyContent: "center", gap: 28, marginBottom: 26 }}>
                {screen.stats.map((s) => (
                  <div key={s.l}>
                    <p className="am-serif" style={{ fontSize: 26, fontWeight: 600, color: "var(--accent)", margin: 0 }}>{s.n}</p>
                    <p className="am-eyebrow" style={{ fontSize: 9 }}>{s.l}</p>
                  </div>
                ))}
              </div>
              <div style={{ border: "1px solid var(--rule)", borderTop: "1px solid var(--ink)", borderRadius: 10, background: "var(--paper)", padding: "20px 22px", maxWidth: 460, margin: "0 auto 28px" }}>
                <p className="am-serif" style={{ fontSize: 16, fontStyle: "italic", color: "var(--ink-deep)", lineHeight: 1.5, margin: "0 0 8px" }}>{screen.quote}</p>
                <p style={{ fontSize: 13, color: "var(--accent)", margin: 0 }}>{screen.cite}</p>
              </div>
              <button onClick={next} className="am-btn">See what&apos;s possible for you &rarr;</button>
              <BackBtn onClick={back} />
            </div>
          )}
          ...
          {screen.type === "email" && (
            <div style={{ textAlign: "center" }}>
              <p className="am-eyebrow" style={{ marginBottom: 14, color: "var(--accent)" }}>Your plan is ready</p>
              <h2 className="am-h2" style={{ fontSize: 30, margin: "0 0 10px" }}>Where should we send your <em className="am-italic">personalized plan</em>?</h2>
              <p style={{ fontSize: 14.5, color: "var(--ink-soft)", fontWeight: 300, margin: "0 0 26px" }}>Get instant access to your custom plan + training video.</p>
                ...
                {[
                  ...
                  <div key={t.name} style={{ border: "1px solid var(--rule)", borderRadius: 10, overflow: "hidden", background: "var(--soft)" }}>
                    ...
                    <div style={{ padding: "10px 12px", textAlign: "left" }}>
                      <p className="am-serif" style={{ fontSize: 14, fontWeight: 500, color: "var(--ink-deep)", margin: 0 }}>{t.name}</p>
              ...
              {(() => {
                const fieldStyle = { width: "100%", boxSizing: "border-box" as const, padding: "15px 16px", fontSize: 15, fontFamily: "var(--sans)", border: "1px solid var(--rule-strong)", borderRadius: 8, marginBottom: 12, color: "var(--ink-deep)", outlineColor: "var(--accent)" };
                return (
                  <>
                    <input type="text" value={name} onChange={(e) => setName(e.target.value)} placeholder="Your name" autoComplete="name" style={fieldStyle} />
                    <input type="email" value={email} onChange={(e) => setEmail(e.target.value)} placeholder="Your best email address" autoComplete="email" style={fieldStyle} />
                    <div style={{ display: "flex", gap: 8, marginBottom: 12 }}>
                      <select value={dialCode} onChange={(e) => setDialCode(e.target.value)} style={{ flexShrink: 0, padding: "15px 10px", fontSize: 15, fontFamily: "var(--sans)", border: "1px solid var(--rule-strong)", borderRadius: 8, color: "var(--ink-deep)", background: "var(--bg)", cursor: "pointer", outlineColor: "var(--accent)" }}>
                        {COUNTRIES.map((c, idx) => <option key={`${c.c}-${idx}`} value={c.d}>{c.f} {c.c} {c.d}</option>)}
                      </select>
                      <input type="tel" value={phone} onChange={(e) => setPhone(e.target.value)} placeholder="Your phone number" autoComplete="tel" style={{ ...fieldStyle, marginBottom: 0, flex: 1 }} />
                    </div>
                  </>
                );
              })()}
              <button onClick={finish} disabled={!leadValid} className="am-btn" style={{ width: "100%", opacity: leadValid ? 1 : 0.4, cursor: leadValid ? "pointer" : "not-allowed" }}>
...
function OptCard({ o, selected, multi, onClick }: { o: Opt; selected?: boolean; multi?: boolean; onClick: () => void }) {
  return (
    ...
    >
      ...
      <span style={{ flexShrink: 0, width: 22, height: 22, borderRadius: multi ? 5 : "50%", border: `2px solid ${selected ? "var(--accent)" : "var(--rule-strong)"}`, display: "flex", alignItems: "center", justifyContent: "center" }}>
        {selected && (multi
          ? <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="var(--accent)" strokeWidth="3.5" strokeLinecap="round"><path d="M5 13l4 4L19 7" /></svg>
...
function BackBtn({ onClick }: { onClick: () => void }) {
  return <button onClick={onClick} style={{ marginTop: 24, background: "none", border: 0, color: "var(--ink-mute)", fontSize: 13, cursor: "pointer", fontFamily: "var(--sans)" }}>&larr; Back</button>;
}
...
function Analyzing() {
  ...
  return (
    <div style={{ textAlign: "center", maxWidth: 440, margin: "0 auto" }}>
      <div style={{ width: 44, height: 44, border: "3px solid var(--rule)", borderTopColor: "var(--accent)", borderRadius: "50%", margin: "0 auto 22px", animation: "amSpin 0.9s linear infinite" }} />
      <h2 className="am-h2" style={{ fontSize: 24, margin: "0 0 24px" }}>Analyzing your physique profile…</h2>
      <div style={{ textAlign: "left", display: "flex", flexDirection: "column", gap: 12 }}>
        {items.map((it, idx) => (
          <div key={idx} style={{ display: "flex", alignItems: "center", gap: 10, opacity: idx < done ? 1 : 0.4, transition: "opacity .3s" }}>
            <span style={{ flexShrink: 0, width: 20, height: 20, borderRadius: "50%", background: idx < done ? "var(--accent)" : "transparent", border: idx < done ? "0" : "2px solid var(--rule-strong)", display: "flex", alignItems: "center", justifyContent: "center" }}>
              {idx < done && <svg width="11" height="11" viewBox="0 0 24 24" fill="none" stroke="#fff" strokeWidth="3.5" strokeLinecap="round"><path d="M5 13l4 4L19 7" /></svg>}
            </span>
            <span style={{ fontSize: 14, color: "var(--ink-soft)" }}>{it}</span>
          </div>
        ))}
      </div>
      <style jsx global>{`@keyframes amSpin { to { transform: rotate(360deg) } }`}</style>
    </div>
  );
}

export default function QuizPage() {
  return <Suspense fallback={<div style={{ minHeight: "100vh", background: "#fff" }} />}><QuizInner /></Suspense>;
}
