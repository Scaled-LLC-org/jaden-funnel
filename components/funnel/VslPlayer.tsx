...
export function VslPlayer({ bookHref }: { bookHref: string }) {
  ...
  const toggle = () => {
    logger.info(paused ? "vsl_play" : "vsl_exit_intent_shown");
    setPaused((p) => !p);
  };
    ...
    <div className="am-vid" style={{ aspectRatio: "16/9", margin: "0 auto", cursor: "pointer" }} onClick={toggle}>
      ...
      {paused && (
        <div className="am-fade" style={{ position: "absolute", inset: 0, background: "rgba(12,20,24,0.93)", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", padding: 24, zIndex: 5 }}>
          ...
          <Link href={bookHref} className="am-btn" onClick={() => logger.info("vsl_cta_click", { source: "exit_intent" })}>Book My Free Call</Link>
        </div>
      )}
    </div>
  );
